...
external unsafeReplaceBy : Js.Re.t ->  (t -> t array -> int -> t -> t [@bs.uncurry]) -> t -> t =
...
let outStr1 = unsafeReplaceBy [%re "/(\\w+)-(\\w+)/"] replaceFunction1 inStr1
let () = Js.log(outStr1)
...
let outStr2 = unsafeReplaceBy [%re "/\\((\\d+), (\\d+)\\) to \\((\\d+), (\\d+)\\)/"] replaceFunction2 inStr2
let () = Js.log(outStr2)
